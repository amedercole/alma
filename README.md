...
| Email      | Resend (HTTPS API); console transport in dev                     |
...
#    set RESEND_API_KEY (see "Email" below); otherwise emails are just printed
...
address they submitted) and a **notification to the attorney**. Email is sent via
**Resend** (HTTPS API) when `RESEND_API_KEY` is set; otherwise messages are
logged to the server console (the local-dev default). To send from your own
address, verify a domain in Resend and point `EMAIL_FROM` at it
...
`STORAGE_LOCAL_DIR`, set the environment variables from `.env.example` (Resend
`RESEND_API_KEY` + `EMAIL_FROM` for email), and deploy. `railway.json` builds
with `npm run build`, and its pre-deploy step runs
